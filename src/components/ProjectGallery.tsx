...
const projects = [
  
  {
    title: "互動式天氣預報 App",
    category: "API 整合應用",
    description: "根據用戶位置顯示及時天氣資訊，學習處理非同步資料請求與動態背景變換。",
    objective: "解決複雜 API 資料處理與非同步狀態管理的挑戰，並實作根據天氣動態變動的沉浸式 UI。",
  ...
  },
  {
    title: "質感高效待辦清單",
    category: "互動介面設計",
    description: "結合瀏覽器儲存技術，打造具備流暢動畫與直覺操作的日常工作管理工具。",
    objective: "探索極簡主義設計與高性能動畫的結合，利用 LocalStorage 確保在離線狀態下也能提供毫秒級響應。",
  ...
  },
  {
    ...
    category: "品牌展示與全端開發",
    description: "為 MG 汽車打造的高級品牌展示門戶，整合動態車款目錄、經銷商地圖與線上預約系統。",
    objective: "利用 React 17 與高級 CSS 技術，將汽車品牌的數位體驗提升至 F1 級別的標準，並整合 RESTful API 實現全端功能。",
    stack: ["React", "Ant Design", "Styled Components", "Node.js", "Express", "PostgreSQL"],
    metrics: [
              ...
              <div className="relative p-8 h-full flex flex-col text-white">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Github size={18} className="text-white hover:text-rbr-yellow" />
                    <ExternalLink size={18} className="text-white hover:text-rbr-yellow" />
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-auto">
                  {project.tags.map((tag) => (
                    <span key={tag} className="text-[10px] px-2 py-1 bg-white/5 border border-white/10 rounded-sm font-mono text-gray-400">
                      {tag}
                    </span>
                  ))}
                </div>
